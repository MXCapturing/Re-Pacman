    ...
    public void ChangePath()
    {
        switch (GameManager.instance.ghostState)
        {
            case GhostState.Chase:
                seeker.StartPath(transform.position, (target.position + (target.right * 2)) + (blinkyAI.path.vectorPath[0] - blinkyAI.transform.position).normalized, OnPathComplete);
